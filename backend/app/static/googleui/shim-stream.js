...
// AIR4: фиксированный сайдбар со списком чатов
(function () {
  if (typeof window === "undefined" || !window.fetch) return;

  function getCurrentSessionId() {
    try {
      const stored = window.localStorage && window.localStorage.getItem("air4.currentSessionId");
      if (stored) return stored;
    } catch {}
    return window.air4CurrentSessionId || "ui";
  }

  function setCurrentSessionId(id) {
    const sid = id || "ui";
    window.air4CurrentSessionId = sid;
    try {
      window.localStorage.setItem("air4.currentSessionId", sid);
    } catch {}
    // подсветка активного
    document.querySelectorAll("#air4-chat-sidebar .air4-chat-row").forEach((row) => {
      if (row.dataset.id === sid) {
        row.style.background = "#111827";
        row.style.borderColor = "#4b5563";
      } else {
  ...
  async function loadSidebarSessions(listEl) {
    ...
    try {
      ...
      const sessions = js.sessions || [];
      if (!sessions.length) {
        listEl.textContent = "Пока нет чатов.";
        return;
      }
      ...
      sessions
        ...
        .forEach((s) => {
          ...
          const title = s.title && s.title !== "New session" ? s.title : s.id;
      ...
      listEl.textContent = "Ошибка: " + e;
    }
  }

  function createChatSidebar() {
    if (document.getElementById("air4-chat-sidebar")) return;

    const bar = document.createElement("div");
    bar.id = "air4-chat-sidebar";
    bar.style.position = "fixed";
    bar.style.top = "64px";
    bar.style.left = "12px";
    ...
    bar.style.boxShadow = "0 4px 24px rgba(0,0,0,0.5)";
    bar.style.fontFamily = "system-ui, -apple-system, BlinkMacSystemFont, sans-serif";
    bar.style.fontSize = "12px";

    const header = document.createElement("div");
    header.textContent = "AIR4 • Чаты";
    header.style.fontWeight = "600";
    header.style.marginBottom = "8px";

    const list = document.createElement("div");
    list.id = "air4-chat-sidebar-list";
    ...
    newBtn.onclick = async () => {
      const ts = Date.now();
      const sid = "chat-" + ts.toString(36);
      setCurrentSessionId(sid);
      await loadSidebarSessions(list);
    };
    ...
    document.body.appendChild(bar);
    loadSidebarSessions(list);
  }

  function bootSidebar() {
    // восстановить текущую сессию из localStorage
